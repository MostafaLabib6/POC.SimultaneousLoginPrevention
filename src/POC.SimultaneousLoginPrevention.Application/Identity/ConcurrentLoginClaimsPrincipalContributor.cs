...
public class ConcurrentLoginClaimsPrincipalContributor : IAbpClaimsPrincipalContributor, ITransientDependency
{
    ...
    public async Task ContributeAsync(AbpClaimsPrincipalContributorContext context)
    {
        ...
        if (userId.HasValue)
        {
            ...
            var token = user.GetProperty(SimultaniousLoginConsts.ConcurrentLoginToken).ToString();
            identity.AddClaim(new Claim(SimultaniousLoginConsts.ConcurrentLoginToken, token));
        }
    }
}
